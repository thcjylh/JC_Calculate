...
md = 1.036  # 密度
zj = 1.38  # 总碱量
cl_content = 0.029  # 氯离子含量

# 拌合物指标
strength_ratio_1d = 186  # 1d抗压强度比
strength_ratio_3d = 174  # 3d抗压强度比
strength_ratio_7d = 132  # 7d抗压强度比
strength_ratio_28d = 129  # 28d抗压强度比
bleed_water_ratio = 46  # 泌水率比
st_diff = 120  # 凝结时间差
ag = 2.8  # 掺外加剂含气量
shrink = 118  # 收缩率比

template_dir = 'E:/文档/10.数据计算/高效减水剂/高效减水剂.xlsx'  # 表格模板路径地址
...
def cl(x_cl_d, sheet_name):  # 单次氯离子含量计算
    ...
    v02 = round(1 / random.randint(995, 1005) * 10000 + v01, 2)  # 20ml 0.1mol/L硝酸钠消耗硝酸银的体积
    if (v02 * 10) - math.floor(v02 * 10) == 0:
        v02 = round(v02 + 0.01, 2)  # 保证V01末尾不为0
    c = f.xy(Decimal(1) / (Decimal(v02) - Decimal(v01)), 0.0001)  # 硝酸银溶液浓度（计算）
    ...
    v02_l.append((math.floor(v02 * 10) - 1) / 10)  # 滴加硝酸银的体积（20ml）初始值
    for a in range(3):
...
def air_content(ag_f_air_c):  # 含气量
    ...
    ag_f_air_c = f.xy(round((Decimal(ag1) + Decimal(ag2)) * 10000 / 2) / 10000, 0.1)  # 未校正含气量
...
md += random.randint(-5, 5) / 1000  # 密度浮动0.005
...
average_hg = f.xy(round(sum(average) / len(average) * 1000) / 1000, 0.01)  # 含固量平均值
...
my_sheet['E17'] = f.xy(round(sum(average) / len(average) * 1000) / 1000, 0.01)  # 硫酸钠含量平均值
...
my_sheet['E21'] = f.xy(round(sum(average) / len(average) * 1000) / 1000, 0.001)  # 密度平均值
# 总碱量计算
...
my_sheet['E36'] = f.xy(round(sum(average) / len(average) * 1000) / 1000, 0.01)  # 总碱量平均值
...
my_sheet0['AZ148'] = f.xy((230 - w) / 230 * 100, 0.1)  # 减水率计算
...
b = 3.4  # 基准泌水率
st = 390  # 基准凝结时间
compressive_strength_1d = 10  # 基准1d抗压强度
compressive_strength_3d = 15  # 基准3d抗压强度
compressive_strength_7d = 22  # 基准7d抗压强度
compressive_strength_28d = 30  # 基准28d抗压强度
for j in range(3):
